    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form id="createContactForm" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Max 5MB. PDF, DOC, DOCX, TXT.</p>
                        ...
                        @forelse($customFieldDefinitions as $definition)
                            <div>
                                ...
                                @if($definition->field_type == 'textarea')
                                    <textarea name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" rows="3"></textarea>
                                @elseif($definition->field_type == 'date')
                                    <input type="date" name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                @elseif($definition->field_type == 'number')
                                    <input type="number" name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                @elseif($definition->field_type == 'checkbox')
                                    <input type="checkbox" name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" value="1" class="mt-1 rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                @elseif($definition->field_type == 'radio')
                                    {{-- For radio, you'd typically need options defined somewhere, for simplicity, we'll assume a text input or a pre-defined set --}}
                                    {{-- For true radio functionality, you'd need more complex custom field definition to store options --}}
                                    <input type="text" name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Enter value">
                                @else {{-- Default to text input --}}
                                    <input type="text" name="custom_fields[{{ $definition->id }}]" id="custom_fields[{{ $definition->id }}]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                @endif
                                <p class="text-sm text-red-600 mt-1" id="custom_fields.{{ $definition->id }}-error"></p>
                            </div>
                        @empty
                            <p class="text-gray-500">No custom fields defined yet. Go to <a href="{{ route('custom-fields.index') }}" class="text-indigo-600 hover:underline">Custom Fields</a> to add some.</p>
                        @endforelse
                    </div>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                alert('Error: ' + (response.message || 'Could not create contact.'));
                            }
                        },
                        error: function(xhr) {
                            if (xhr.status === 422) { 
